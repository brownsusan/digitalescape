...
{	
	...
	$(".header-learn-more").click(function(e){
		e.preventDefault;
		$('html, body').animate({
			scrollTop: $("#page-features").offset().top - 20
		}, 2000);
	});
...
;(function(factory){
  ...
  var Zippy = (function(element, settings){
    ...
    function _Zippy(element, settings){
      ...
      this.defaults = {
        slideDuration: '3000',
        speed: 500,
        arrowRight: $(element).find('.game-art-arrow-right'),
        arrowLeft: $(element).find('.game-art-arrow-left')
      };
  ...
  /**
   ...
   Zippy.prototype.init = function(){
	...
	Zippy.prototype.addCSSDuration = function(){
		var _ = this;
		this.$el.find('.game-art-slide').each(function(){
   ...
   */
	...
	Zippy.prototype.build = function(){
		...
		var $indicators = this.$el.append('<ul class="game-art-indicators" >').find('.game-art-indicators');
		this.totalSlides = this.$el.find('.game-art-slide').length;
	...
	Zippy.prototype.activate = function(){
		this.$currSlide = this.$el.find('.game-art-slide').eq(0);
		this.$el.find('.game-art-indicators li').eq(0).addClass('active');
	};
   ...
   */
	Zippy.prototype.events = function(){
		$('body')
			...
			.on('click','.game-art-indicators li',this.changeSlide);
	};
	...
	Zippy.prototype.changeSlide = function(e){
		...
		var $nextSlide = this.$el.find('.game-art-slide').eq(this.currSlide).addClass(direction + ' active');
    ...
    //Init CSS transitions
		setTimeout(function(){
			this.$el.addClass('game-art-transition');
			this.addCSSDuration();
		...
		//Start timer
		setTimeout(function(){
			this.$el.removeClass('game-art-transition');
  ...
  /**
	...
	Zippy.prototype._updateIndicators = function(){
		this.$el.find('.game-art-indicators li').removeClass('active').eq(this.currSlide).addClass('active');
	};
...
$('.game-art-carousel').each(function(){
	var id = $(this).attr('id');
	
	var args = {
		arrowRight : '#'+id+' .game-art-arrow-right', //A jQuery reference to the right arrow
		arrowLeft : '#'+id+' .game-art-arrow-left', //A jQuery reference to the left arrow
		speed : 500, //The speed of the animation (milliseconds)
		slideDuration : 4000 //The amount of time between animations (milliseconds)
	};

	$('#'+id+'.game-art-carousel').Zippy(args);
});
