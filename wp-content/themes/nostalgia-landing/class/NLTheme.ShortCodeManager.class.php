...
class NLThemeShortCodeManager
{
	...
	function NLThemeShortCodeManager()
	{
	...
	function createManagerSelectShortCodeList()
	{
		...
		$html.='<option value="shortcode">['.esc_html__('Select shortcode',THEME_DOMAIN).']</option>';
	...
	function createManagerPanel()
	{
		...
		foreach($this->shortCodeData as $shortCode=>$shortCodeData)
		{
			...
			NLThemeHelper::removeUIndex($shortCodeData,'description','hidden_content','section');
			...
			if(is_array($shortCodeData['section']))
			{
				foreach($shortCodeData['section'] as $section)
				{
					...
					{
						NLThemeHelper::removeUIndex($fieldData,'legend','html_attribute');
					...
					NLThemeHelper::removeUIndex($section,'header','legend');
