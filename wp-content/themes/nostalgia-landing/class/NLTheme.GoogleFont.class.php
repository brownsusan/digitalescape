...
class NLThemeGoogleFont
{
	...
	{
		$cTime=time();
		$oTime=(int)NLThemeOption::getOption('font_google_time');
	...
	function download()
	{
		...
		NLThemeOption::updateOption(array('font_google_time'=>time()));
	...
	function setToStyle(&$style)
	{
		...
		$download=array();
		$selector=array('base','h1','h2','h3','h4','h5','h6','subheader','link_button');
	
		$dictionary=$this->unpack();

		foreach($selector as $value)
		{
			$name=NLThemeOption::getOption('font_font_family_google_'.$value);
